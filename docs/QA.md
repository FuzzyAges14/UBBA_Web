...
| API | Vitest + fetch / app harness | `/api/leads` validation, honeypot, CORS, rate limit, payload size, delivery failure |
...
- SEO: title/description/canonical/OG/Twitter via `src/data/seo.ts`; no duplicate tags; program + location landings
...
- Accessibility: axe on major routes (incl. location); form label associations; skip link; single homepage H1
- API HTTP: valid lead, missing fields, invalid email, honeypot, oversized body, CORS allow/deny, rate limit 429, email failure, health
- E2E: critical flows (see `e2e/critical-flows.spec.ts`)

## Known untested / deferred areas

| Area | Why |
| --- | --- |
| Full visual QA of `/locations/*` landings | Covered in Vitest route/H1 + SEO tests; E2E still focuses Home `#locations` |
