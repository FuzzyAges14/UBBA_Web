...
| Area | Why |
| --- | --- |
| Dedicated `/locations/*` pages | Not in the current app (locations live on Home `#locations` + Contact) |
| Rate limiting `429` | Pending Backend Security agent; tracked as `it.todo` in `server/app.test.ts` |
| Full Open Graph / Twitter tags | Current `Seo` only sets title + description |
